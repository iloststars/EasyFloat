...
internal class FloatingWindowHelper(val context: Context, var config: FloatConfig) {
    ...
    private fun setChangedListener() {
        frameLayout?.apply {
            ...
            viewTreeObserver?.addOnGlobalLayoutListener {
                ...
                // 更新浮窗位置信息
                windowManager.updateViewLayout(frameLayout, F.getFakeRecorderWindowLayoutParams(params))
            }
        }
    }
    ...
    private fun setGravity(view: View?) {
        ...
        // 更新浮窗位置信息
        windowManager.updateViewLayout(view, F.getFakeRecorderWindowLayoutParams(params))
    }
    ...
    fun updateFloat(x: Int = -1, y: Int = -1, width: Int = -1, height: Int = -1) {
        frameLayout?.let {
            if (x == -1 && y == -1 && width == -1 && height == -1) {
                ...
                windowManager.updateViewLayout(it, F.getFakeRecorderWindowLayoutParams(params))
            }
        }
    }
}
